...
class FOV(): 
    def __init__(self, game_board, actor_board): 
        self.game_board = game_board 
        self.actor_board = actor_board 
        self.RAYS = 360 
        self.STEP = 3 
        self.sintable = [
    ...
    def update(self, entity_pos, vision_range): 
        ...
        for i in range(0, self.RAYS, self.STEP): 
            ...
            for z in range(vision_range): 
                ...
                y += angle_y 
                try: 
                    self.game_board[int(round(x))][int(round(y))].revealed = True 
                    self.visible_tiles.append(self.game_board[int(round(x))][int(round(y))]) 
                    if type(self.actor_board[int(round(x))][int(round(y))]) != int: 
                        self.actor_board[int(round(x))][int(round(y))].revealed = True 
                        self.visible_tiles.append(self.actor_board[int(round(x))][int(round(y))]) 
                    #self.game_board[int(round(x))][int(round(y))].revealed = False 
                    if self.game_board[int(round(x))][int(round(y))].blocks_sight or self.actor_board[int(round(x))][int(round(y))].blocks_sight: 
                        break 
                except: 
                    pass 